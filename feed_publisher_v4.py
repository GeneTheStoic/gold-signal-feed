"""
feed_publisher_v4.py

Daily feed publisher for the gold options signal utility (Product #2).
GitHub Actions edition.

v4 CHANGE: feed schema bumped to 2. The entry-stack decision logic is
IDENTICAL to v3 (EA-parity, validated funded configuration). What changed
is what gets PUBLISHED: the feed now carries the four directed z-scores,
the readiness score, and the first gate that blocked (if any), so the
chart panel can show honest diagnostics instead of a bare BUY/NONE.

FEED FORMAT schema 2 (exactly 2 lines, comma separated, 14 fields):
  schema,date,signal,ensemble,z_vrp,z_skew,z_pcr,z_term,score,
  regime_favorable,macro_favorable,trend_er,blocked_by,generated_utc

  z_* values are DIRECTED (direction already applied: positive = bullish
  contribution) so the panel can render them without knowing the mapping.
  blocked_by is one of: NONE SCORE MACRO VRP RSI CRASH REGIME AGREE THRESHOLD
...
USE_VRP_GATE            = False    # live funded config: OFF
VRP_MIN_Z               = 0.0
SCORE_LOOKBACK          = 10
...
def decide_signal(valid):
    """Long-entry stack, gate for gate.
    Returns (signal, blocked_by, score, log_lines)."""
    log = []
    row = valid.iloc[-1]
    ...
    if score < MIN_DEPLOY_SCORE:
        return "NONE", "SCORE", score, log + ["BLOCKED by score gate"]

    log.append(f"macro gate: macro_favorable={int(macro_ok)}")
    if not macro_ok:
        return "NONE", "MACRO", score, log + ["BLOCKED by macro gate"]

    if USE_VRP_GATE:
        log.append(f"vrp gate: z_VRP {z_vrp:+.2f} need > {VRP_MIN_Z}")
        if z_vrp <= VRP_MIN_Z:
            return "NONE", "VRP", score, log + ["BLOCKED by VRP gate"]
    else:
    ...
    if filtered > 0 and rsi < rsi_min:
        return "NONE", "RSI", score, log + ["BLOCKED by RSI gate"]
    ...
    if atr > 0 and day_range > atr * FLASHCRASH_ATR_MULT:
        return "NONE", "CRASH", score, log + ["BLOCKED by flash-crash filter"]

    if not regime_ok:
        return "NONE", "REGIME", score, log + ["BLOCKED by regime gate"]
    ...
    if agreeing < MIN_SIGNALS_REQUIRED:
        return "NONE", "AGREE", score, log + ["BLOCKED by agreement check"]

    if filtered == 0.0 or filtered < long_thr:
        return "NONE", "THRESHOLD", score, \
               log + [f"BLOCKED: filtered {filtered:+.2f} < threshold {long_thr:.2f}"]

    return "BUY", "NONE", score, log + ["ALL GATES PASSED -> BUY"]
...
def build_feed_row(df):
    ...
    signal, blocked_by, score, log = decide_signal(valid)
    ...
    # Directed z-scores: positive = bullish contribution
    zd_vrp  = float(row["zscore_VRP"])  * SIGNAL_DIRECTIONS["VRP"]
    zd_skew = float(row["zscore_SKEW"]) * SIGNAL_DIRECTIONS["SKEW"]
    zd_pcr  = float(row["zscore_PCR"])  * SIGNAL_DIRECTIONS["PCR"]
    zd_term = float(row["zscore_TERM"]) * SIGNAL_DIRECTIONS["TERM"]
    ...
    header = ("schema,date,signal,ensemble,z_vrp,z_skew,z_pcr,z_term,score,"
              "regime_favorable,macro_favorable,trend_er,blocked_by,generated_utc")
    line = (f"{SCHEMA_VERSION},{row_date.strftime('%Y.%m.%d')},{signal},"
            f"{ensemble:.4f},{zd_vrp:.4f},{zd_skew:.4f},{zd_pcr:.4f},{zd_term:.4f},"
            f"{score:.1f},{regime},{macro},{trend_er:.4f},{blocked_by},{generated}")
...
if __name__ == "__main__":
    print("=" * 60)
    print("Gold Signal Feed Publisher v4 (schema 2 diagnostics)")
    print("=" * 60)
    try:
        data = run_pipeline()
